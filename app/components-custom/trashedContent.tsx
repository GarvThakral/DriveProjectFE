...
export default function TrashContents({ setDragOver , params}: { setDragOver: (over: boolean) => void , params:string[]}) {
  const { view } = useView();
  ...
  useEffect(() => {
    const fetchContent = async () => {
      if (!token) {
        ...
        const filesArray = await axios.get(`http://localhost:3002/files/trash`, { headers: { token } });
